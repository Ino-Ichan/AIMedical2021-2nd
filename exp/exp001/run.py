...
class AIMedicalModel(nn.Module):
    def __init__(self, in_channel=12, ninp=512, nhead=4, nhid=1024, dropout=0.1, nlayers=1):
        ...
        self.top_layer3 = nn.Sequential(
                             nn.Conv1d(in_channel, d_model//4, kernel_size=21, padding=10),
                             nn.ReLU(inplace=True),
                         )
        
        self.top_layer4 = nn.Sequential(
                             nn.Conv1d(in_channel, d_model//4, kernel_size=31, padding=15),
                             nn.ReLU(inplace=True),
                         )
